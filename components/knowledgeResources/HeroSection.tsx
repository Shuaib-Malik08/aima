...
function HeroSection({ block }: { block?: KnowledgeHeroBlock }) {
  const banner = block?.data?.banners?.[0];

  const fullTitle =
    banner?.title ?? block?.data?.title ?? "Knowledge Resources";
  const description =
    banner?.description ??
    "Industry and Academic Based Research, Reports and Management Articles";
  const bannerImage =
    banner?.image ?? "knowledge_resources_img/knowledgebanner.png";
