...
function EventHeroSection({ block }: { block: EventHeroBlock }) {
  const banner = block?.data?.banners?.[0];
  const bgImage = banner?.image;
  const fullTitle = banner?.title ?? block?.data?.title ?? "";
  const description = banner?.description ?? "";

  // Split title: everything except last word is white, last word is gold
